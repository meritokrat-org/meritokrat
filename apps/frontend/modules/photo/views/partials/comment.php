<? $comment = photo_comments_peer::instance()->get_item($id) ?>
	...
	<div class="left ml10" style="width: 615px;">
		...
		<div class="fs11 mb5 mt5">
			...
			<? if ( session::has_credential('moderator') || ( ( $comment['user_id'] == session::get_user_id() ) && !$comment['childs'] ) ) { ?>
				<a href="javascript:;" onclick=" if ( confirm('Точно?') ) { $(this).parent().parent().parent().hide(); $.get('/photo/delete_comment?id=<?=$comment['id']?>') } " class="dotted ml10"><?=t('Удалить')?></a>
			<? } ?>
		</div>
	</div>
	<div class="clear"></div>
