...
<!--<div class="mt5" style='width:755px;height:135px; background: url("https://meritokrat.org/static/images/common/pl_bg1.png") repeat scroll 3% 0% transparent;'>
    ...
    <div style="margin-left:6px;margin-top:20px" class="left">
        <a href="/photocompetition/<?//=$kzz?>" class="pointer">
...
<div class="mt10 hide">
	<a href="/profile/desktop?id=<?=session::get_user_id()?>"><img src="/static/images/vnesok<? if(translate::get_lang() == 'ru'){ ?>_ru<? } ?>.gif" /></a>
</div>
...
<div class="left" style="width: 35%;">
		...
		<!--<div class="column_head mt10" >
			<b class="left" style="cursor: pointer;" onclick="Application.ShowHide('declarations')"><?=t('Объявления')?></b>
		</div>
        <? if (session::has_credential('admin')) { ?>
                <div class="box_content p5">
                        <a class="fs11" href="/blogs/edit?type=<?=blogs_posts_peer::TYPE_DECLARATION_POST?>"><?=t('Добавить объявление')?> &rarr;</a>
                </div>
        <? } ?>
            ...
            <div class="mb5 mt5" id="new_events">
                    ...
                    <div class="mb5 p5 box_content">
                        <a href="/event<?=$event['id']?>" style="font-size:17px;" class="a_left bold"><?=stripslashes(htmlspecialchars($event['name']))?></a>
                        ...
                        <?switch($event['type'])
                        {
                          ...
                          case 4:
                                ...
                                if($event['content_id']>0)$ppo=ppo_peer::instance()->get_item($event['content_id']);
                                 ?>
                                 <a href="/ppo<?=$ppo['id']?>/<?=$ppo['number']?>" style="color:black"><?=$ppo['title']?></a> (<?=user_helper::full_name($event['user_id'],true,array('style'=>'color:black'),false);?>)
                                 <?
                                break;
                            case 3:
                                ?>
                    ...
                    <?if($event['status']==1 || $event['status']==3){?><span class="fs11 green"><?=($event['status']==1)?t('Вы посещаете это мероприятие'):t('Вы возможно посещаете это мероприятие')?></span><?}?>
        ...
        <? if (session::has_credential('admin')) { ?>
                <div class="box_content p5">
                        <a class="fs11" class="bold a_left" href="/blogs/edit?type=<?=blogs_posts_peer::TYPE_NEWS_POST?>"><?=t('Добавить новость')?> &rarr;</a>
                </div>
        <? } ?>
			...
			<b class="left" style="margin-top:3px;"><?=t('Новости сообществ')?></b>
                ...
                <? if ( $group_news ) { ?>
                        <? foreach ( $group_news as $id ) { ?>
                                ...
                                <div class="mb10 p10 box_content">
                                        <span class="quiet fs11"><?=date_helper::human($data['created_ts'], ', ')?></span><br>
                                        <a href="/groups/newsread?id=<?=$id?>" class="bold a_left"><?=stripslashes(htmlspecialchars($data['title']))?></a>
                                </div>
                        <? } ?>
                <? } ?>
                </div>
            ...
            <div class="column_head mt10" style="cursor: pointer;" onclick="Application.ShowHide('groups')">
                    <b class="left" style="margin-top:3px;"><?=t('Новые сообщества')?></b>
										<a class="right" style="margin-top:3px;" rel="nofollow" href="/groups"><?=t('Все')?> &rarr;</a>
            </div>
                        ...
                        <div class="p10 box_content mb10">
                                ...
                                <div class="left ml5" style="margin-bottom: -5px; width: 145px; line-height: 130%;">
                                        ...
                                        <div class="fs11 quiet">
                                            <?=($group['category']!=2)?groups_peer::instance()->get_type($group['type']):t('Рабочая группа')?>
                                        </div>
                                </div>
                                <div class="clear"></div>
                        </div>
...
<div id="attention" class="left ml10" style="width: 100%;">
	<div class="column_head mt10">
            ...
            <a class="right fs10 mr5" id="close_attention" href="javascript:;"><?=t('Закрыть окно')?>&nbsp;<?=tag_helper::image('icons/x.png',array('style'=>'cursor: pointer;'))?></a>
	</div>
    <div class="left" style="background-color:#FAE1A8;width:100%;font-size: 13px;line-height:1.2;">
        <div class="ml10 mr5 mt10"><?=session::get('language')=='ru' ? $attention['anounces_ru'] : $attention['anounces']?>
            <div class="right mr5">
                <a class="fs11 cgray" href="/home/attention?id=<?=$attention['id']?>"><?=t('Читать дальше')?> &rarr;</a>
            </div>
        </div>
    </div>
</div>
...
<div class="left ml10" style="width: 100%;">
	<div class="column_head mt10 mb10" style="cursor: pointer;" onclick="Application.ShowHide('blogs')">
		<b class="left" style="margin-top:3px;"><?=t('Последние публикации')//.t('Интересная информация')?></b>
...
<div class="left ml10" style="width: 100%;">
	<div class="column_head mt10" style="cursor: pointer;" onclick="Application.ShowHide('reports')">
		...
		<a class="right" style="margin-top:3px;" rel="nofollow" href="/eventreport/show"><?=t('Все')?> &rarr;</a>
	</div>
	<? if(session::has_credential('admin')){ ?>
	<div
		 style="
...
<div class="left ml10" style="width: 100%;">
	<div class="column_head mt10 mb10" style="cursor: pointer;" onclick="Application.ShowHide('blogs_last_wr')">
		<b class="left" style="margin-top:3px;"><?=t('Последние записи в блогах')//.t('Интересная информация')?></b>
