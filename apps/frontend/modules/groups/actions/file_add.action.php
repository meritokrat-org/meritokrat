...
class groups_file_add_action extends groups_controller
{
	...
	{ 
            ...
            if ( !$this->group = groups_peer::instance()->get_item($group_id) )
                    {
                            $this->redirect('/groups/'.$group_id);
                    }
                    
            if (!groups_peer::instance()->is_moderator($group_id, session::get_user_id()) && !session::has_credential('admin') )
                    {
                            throw new public_exception(t('Недостаточно прав'));
                    }

		load::model('groups/files_dirs');
		$dirs = groups_files_dirs_peer::instance()->get_list();
                        ...
                        if (isset($_FILES['file']))
                            {
                                ...
                                foreach ( $_FILES['file']['tmp_name'] as $i => $file )
                                {
                                        ...
                                        if (!groups_files_peer::instance()->is_allowed_filetype($_FILES['file']['name'][$i],  $_FILES['file']['type'][$i]) && !session::get_user_id()==5) throw new public_exception('error in file extension or mime-type: '.stripslashes(htmlspecialchars($_FILES['file']['type'][$i])));
//
                                ...
                                $id = groups_files_peer::instance()->insert(array(
                                                'dir_id' => $dir_id,
                                                'user_id' => session::get_user_id(),
                                                'group_id' => $group_id,
                                                'url' => $_FILES['file']['name'][0],
                                                'size' => groups_files_peer::formatBytes($_FILES['file']['size'][0]),
                                                'title' => $title,
                                ...
                                if ( strlen($_POST['url'])>3 )
                                    {
                                        if (mb_substr($_POST['url'],0,7)!='http://') $_POST['url']='http://'.$_POST['url'];
                                            groups_files_peer::instance()->get_icon(end(explode('.', $_POST['url'])))!=='unknown.png' ? $ext=end(explode('.', $_POST['url'])) : $ext=NULL;
			...
			/*feed_peer::instance()->add(session::get_user_id(), $readers, array(
				...
				'section' => feed_peer::SECTION_PERSONAL
                        ...
                        */
			$this->redirect('/groups/file?id='.$group_id);
	}
}
