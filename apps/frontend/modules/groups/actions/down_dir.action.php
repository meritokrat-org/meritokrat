...
class groups_down_dir_action  extends groups_controller
{
	...
	public function execute()
	{
		...
		if ( request::get('dir_id') )
			...
			$dir_id = request::get_int('dir_id');
                        ...
                        $dirs = groups_files_dirs_peer::instance()->get_list(array('group_id'=>$dir_data['group_id']), array(), array('position DESC'));
...
//информация о папке выше
			...
			$dir = db::get_row( $sql, $bind, null );
                        ...
                        if ($position>0)
                        {
                        db::exec("UPDATE groups_files_dirs SET position=position-1 WHERE position=:position AND group_id=:group_id", array('position'=>$position,'group_id'=>$dir_data['group_id']));
