<h2 class="mt10 mr10 column_head">
	<a href="/groups/position?id=<?=$group['id']?>"><?=t('Позиция МПУ')?></a>
	&rarr;
	<?=t('Просмотр')?>
</h2>
...
<h1 class="mt5 mb10">
	...
	<? if ( ( $topic['user_id'] == session::get_user_id() ) || groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) ) { ?>
		<a href="javascript:;" class="fs11 maroon" onclick="groupsController.deletePositionTopic(<?=$topic['id']?>);"><?=t('Удалить тему')?></a>
	<? } ?>
</h1>
<?  $counter=0;
    foreach ( $list as $id ) {
...
<? if ( session::is_authenticated() ) { ?>
	<form class="form_bg mr10 fs12 mb10" action="/groups/position_message?id=<?=$topic['id']?>" id="message_form">
