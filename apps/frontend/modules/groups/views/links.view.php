...
<? if ( (groups_members_peer::instance()->is_member($group['id'], session::get_user_id()) ) or session::has_credential('admin')) { ?>
	<div class="form_bg mr10 fs12 p10 mb10">
		<div class="left">
			<? if ( $dir_id !== null ) { ?>
				<a href="/groups/link?id=<?=$group['id']?>"><?=t('Папки сообщества')?></a> &rarr;
		...
		<div class="clear"></div>
			...
			<tbody><tr>
                ...
                <div id="add_file" class="<?=request::get('add')==1 ? 'hidden' : ''?>" style="display: none;">
                    <form action="/groups/link_add?id=<?=$group['id']?>" id="photo_form" class="form" method="post" enctype="multipart/form-data">
...
<? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) and $dir_id!=0)   { ?>
                        <div class="left ml15"><a class="fs10" onclick="return confirm('Ви впевнені?');" href="/groups/linkdir_delete?id=<?=$dir_id?>">[X]</a></div>
                    <? } ?>
            <div class="clear"></div>
            ...
            <div class="clear"></div>
                <? foreach ( $links[$dir_id] as $id ) {
                       $link=groups_links_peer::instance()->get_item($id); ?>
                        ...
                        <? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()))   { ?>
                        <div class="left ml15"><a onclick="return confirm('Ви впевнені?');" href="/groups/link_delete?id=<?=$id?>">[X]</a></div>
                    <? } ?>
