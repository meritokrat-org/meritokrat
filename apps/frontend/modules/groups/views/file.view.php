...
<?   $max=db::get_row("SELECT max(position) FROM groups_files_dirs WHERE group_id=:group_id",array('group_id'=>$group['id']));    ?>
	<div class="form_bg mr10 fs12 p10 mb10">
		<div class="left" style="color:#333333; text-align: justify;">
		</div>
		<div class="left" style="color:#333333; text-align: justify;">
                    <a id="showall" class="pointer hidden"><?=t('Развернуть все')?></a>
                    <a id="hideall" class="pointer"><?=t('Свернуть все')?></a>
		</div>
            <div class="right">
            <?  if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id())) { ?><a onclick="$('#add_photoalbum').show(50);" href="javascript:;"><?=t('Добавить папку')?></a>&nbsp;|&nbsp;<? } ?>

            <? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) || (groups_members_peer::instance()->is_member($group['id'], session::get_user_id() ) && !$group['private'])) { ?>
		...
		<table width="100%" class="fs12">
			<tr>
                ...
                <div id="add_file" class="<?=request::get('add')==1 ? 'hidden' : ''?>" style="display: <?=request::get_int('add')!=1 ? 'none' : 'block'?>;">
                    <form action="/groups/file_add" id="photo_form" class="form" method="post" enctype="multipart/form-data">
			<input type="hidden" value="<?=$group['id']?>" name="id" id="id">
		<table width="100%" class="fs12">
			<tr>
			...
			<tr>
                ...
                <div id="add_link" class="hidden" style="display: none;">
                    <form action="/groups/file_add" id="link_form" class="form" method="post">
			<input type="hidden" value="<?=$group['id']?>" name="id" id="id">
		<table width="100%" class="fs12">
			<tr>
				<td class="aright"><?=t('Название')?></td>
...
<? } ?>
  ...
  <?  }   ?>
            ...
            <? //if (session::get_user_id()==29) var_dump(groups_files_peer::instance()->get_list(array('dir_id'=>0,'group_id'=>$group['id']),array(),array('position ASC')));     ?>
            <? foreach ( groups_files_peer::instance()->get_list(array('dir_id'=>0,'group_id'=>$group['id']),array(),array('position ASC')) as $id ) {
                    $counter++;
                    ...
                    <div class="mt5" style="border-bottom: 1px solid #f7f7f7;">
                        <div class="left">
                           <div class="ml5">
                               <? if(isset($file['files'])){ ?>
                               <a href="<?=context::get('file_server').$file['id'].'/'.$arr[0]['salt']."/".$arr[0]['name']?>">
                                    <?=stripslashes(htmlspecialchars($file['title']))?>
                               </a>
                               <? }else{ ?>
                ...
                <?      }      
                            ...
                            <div class="right aright mr5" style="border-bottom: 1px solid #d7d7d7; color:#565656;"> <?=$file['size'] ? $file['size'] : '' //$file['exts'] ? groups_files_peer::formatBytes(filesize($file['url'])) : ''?>
                    ...
                    <? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()))   { ?>
                            ...
                            <? }  if($counter!=count($files[$dir_id])) { ?>
                                ...
                                <? } ?>
                   <? } ?>
                   <? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) || $file['user_id']==session::get_user_id()) { ?>
