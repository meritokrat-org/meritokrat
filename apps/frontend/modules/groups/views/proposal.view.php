<h1 class="mt10 mr10 column_head">
	<a href="/group<?=$group['id']?>"><?=htmlspecialchars($group['title'])?></a> &rarr; <?=t('Предложения')?>
</h1>

<? if ( session::is_authenticated() ) { ?>
<form class="form_bg mr10 fs12 p10 mb10" action="/groups/proposal_create?id=<?=$group['id']?>" method="post" id="topic_form">
    <? if ( !request::get_int('add')) { ?>
		<div class="left">
			<a class="filter <?=!$filter ? ' filter_selected' : ''?>" href="proposal?id=<?=$group['id']?>"><?=t('Новые Темы')?></a>
			<a class="filter <?=$filter == 'hot' ? ' filter_selected' : ''?>" href="proposal?id=<?=$group['id']?>&filter=hot"><?=t('Самые обсуждаемые')?></a>
		</div>
...
<? if ( $list and !request::get_int('add')) { ?>
	<? foreach ( $list as $id ) { include 'partials/proposal.php'; } ?>
