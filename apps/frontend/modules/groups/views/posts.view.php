<div class="mt10 mr10">
	<h1 class="column_head">
		<div class="left">
				<a href="/group<?=$group['id']?>"><?=stripslashes(htmlspecialchars($group['title']))?></a> &rarr; 
				<a href="/groups/posts?group_id=<?=$group['id']?>"><?=t('Мысли')?></a>
			&rarr; <?=t('Список')?>
		</div>
		<div class="clear"></div>
	</h1>
...
<? if (groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) || ($is_member && !$group['private'])) { ?>
<div class="box_content p5 mb10 fs11 mr10">
    <a href="/groups/post_edit?group_id=<?=$group['id']?>&add=1"><?=t('Добавить мысль')?> &rarr;</a>
</div>
<? } ?>
    <? if ( !$posts ) { ?>
            ...
            <div class="left ml10" style="width: 525px;">
                            ...
                            <div class="fs11 pb5">
                                    <div class="left quiet">
                                            ...
                                            <?=user_helper::full_name($post['user_id'],false,array('class'=>'mr10 fs11'),false)?> <a href="/groups/post?group_id=<?=$group['id']?>&id=<?=$id?>"><?=t("Читать дальше")?>  &rarr;</a>
                                    </div>
                            </div>
            </div>
