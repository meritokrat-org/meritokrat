...
<div class="mr10">
    ...
    <a href="javascript:;" onclick="if ( confirm('<?=t('Удалить новость?')?>') ) groupsController.deleteNews(<?=$news_item['id']?>,'/groups/news?id=<?=$group['id']?>');" class="dotted ml10"><?=t('Удалить')?></a>
    <a class="ml10" href="/groups/edit_news?id=<?=$news_item['id']?>" id="newsedit"><?=t('Редактировать')?></a>
<?}?>
		</div>
		<div class="fs12" id="news_body_<?=$id?>">
               ...
               echo image_helper::photo($news_item['photo'], 's', 'groupnews', array('class' => 'border1'));
        ?>
                            </div>
			<? $news_item['text'] = html_entity_decode(text_helper::smart_trim(strip_tags($news_item['text'],'<br><br />'), 320)) ?>
			<p><?=user_helper::get_links(stripslashes($news_item['text']),false)?></p>
                        <a class="right" href="/groups/newsread?id=<?=$news_item['id']?>"><?=t('Читать дальше')?> &rarr;</a>
		</div>
                        <div class="clear"></div>
	<? } ?>
