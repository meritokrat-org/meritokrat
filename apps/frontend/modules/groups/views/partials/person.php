		...
		<div style="height: 55px;">
			...
			<span class="fs11 quiet">
                                ...
                                <? if(session::has_credential('admin')){ ?>
                                    <?=', *'.groups_members_peer::instance()->get_type($group['id'],$id)?>
                                <? } ?>
                        </span>
		</div>
		<div class="fs11">
            ...
            <? if (session::has_credential('admin') || groups_peer::instance()->is_moderator($group['id'], session::get_user_id())) { ?>
		<div class="fs11">
			<a href="/groups/delete_user?group_id=<?=$group['id']?>&id=<?=$id?>"><?=t('Удалить')?></a>
		</div>
            <? } ?>
	</div>
	<div class="clear"></div>
