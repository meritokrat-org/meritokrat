	...
	<div style="margin-left: 60px;">
		...
		<div class="right">
			<? if ( session::is_authenticated() && !blogs_comments_peer::instance()->has_rated($child_id, session::get_user_id()) ) { ?>
				<span>
					<a href="javascript:;" onclick="groupsController.rateComment(this, <?=$child_comment['id']?>, true);"><?=tag_helper::image('common/up.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
					<a href="javascript:;" onclick="groupsController.rateComment(this, <?=$child_comment['id']?>, false);"><?=tag_helper::image('common/down.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
				</span>
			<? } ?>
		...
		<? if ( session::is_authenticated() && !$ajaxaction ) { ?>
			...
			( groups_peer::instance()->is_moderator($group['id'], session::get_user_id()) || session::has_credential('selfmoderator') && $post_data['user_id'] == session::get_user_id() ) ) { ?>
