    ...
    <div class="maximize d-flex flex-row">
        ...
        <div style="width: 224px;">
            ...
            <div class="fs11">
                ...
                <?php if (user_auth_peer::getStatus($user_auth['status_functionary'], user_auth_peer::STATUS_TYPE_FUNCTIONARY)) {
                    $collection[] = user_auth_peer::getStatus($user_auth['status_functionary'], user_auth_peer::STATUS_TYPE_FUNCTIONARY);
                } ?>

                <?php if (user_auth_peer::getStatus($user_auth['status_politician'], user_auth_peer::STATUS_TYPE_POLITICIAN)) {
                    $collection[] = user_auth_peer::getStatus($user_auth['status_politician'], user_auth_peer::STATUS_TYPE_POLITICIAN);
                } ?>
            ...
            <?php $row = db::get_row('select p.id, p.title, pm.function from ppo_members pm join ppo p on p.id = pm.group_id where pm.user_id = :userId limit 1;', ['userId' => $user_auth['id']]) ?>
            <?php if ($row) { ?>
                <div class="fs11">
            ...
            <?php if ($user_data['country_id']) { ?>
                ...
                <?php $geoParts = [
                        ['href' => sprintf('/search?submit=1&country=%s', $user_data['country_id']), 'text' => $country['name_' . translate::get_lang()], 'style' => 'padding-right: 1px'],
                ] ?>
                <?php if ($user_data['region_id']) {
                    $region     = geo_peer::instance()->get_region($user_data['region_id']);
                    $geoParts[] = ['href' => sprintf('/search?submit=1&country=%s&region=%s', $user_data['country_id'], $user_data['region_id']), 'text' => $region['name_' . translate::get_lang()], 'style' => 'padding: 0 1px'];
                } ?>
                <?php if ($user_data['city_id']) {
                    $city       = geo_peer::instance()->get_city($user_data['city_id']);
                    $geoParts[] = ['href' => sprintf('/search?submit=1&country=%s&region=%s&city=%s', $user_data['country_id'], $user_data['region_id'], $user_data['city_id']), 'text' => $city['name_' . translate::get_lang()], 'style' => 'padding-left: 1px'];
                } ?>
                <div class="fs11">
                    <?= implode(
                            '/',
...
HTML;
            ...
            <div class="fs11">
                <?php if (request::get_int('function') > 0) { ?>
                    ...
                    <?php foreach (user_auth_peer::get_functions() as $function_id => $function_title) { ?>
                        <?php if (in_array($function_id, explode(',', str_replace(['{', '}'], ['', ''], $user_desktop['functions'])))) { ?>
        ...
        <div class="fs11" style="width: 140px; text-align: right;">
            ...
            <?php if (request::get_int('list') && (session::has_credential('admin') || (in_array(session::get_user_id(), $own_lists) || in_array(session::get_user_id(), $edit_lists)))) { ?>
                <div>
            ...
            <?php if (request::get('bookmark')) { ?>
                <?php if (request::get('bookmark') == 6) { ?>
                    <?php $bkm = bookmarks_peer::instance()->is_bookmarked(session::get_user_id(), 6, $user_data['user_id']); ?>
                    <a class="right fs10"
                ...
                <?php } else { ?>
                    <?php $bkm = bookmarks_peer::instance()->is_bookmarked(session::get_user_id(), 2, $user_data['user_id']); ?>
                    <a class="bookmark mb10 ml5 right" style="<?= ($bkm) ? 'display:none;' : 'display:block;' ?>"
                       href="#add_bookmark"
                       onclick="Application.bookmarkThisItem(this,'2','<?= $user_data['user_id'] ?>');return false;"><b></b><span><?= t('В закладки') ?></span></a>
                    <a class="unbkmrk mb10 ml5 right" style="<?= ($bkm) ? 'display:block;' : 'display:none;' ?>"
                       href="#del_bookmark"
                       onclick="Application.unbookmarkThisItem(this,'2','<?= $user_data['user_id'] ?>');return false;"><b></b><span><?= t('Удалить из закладок') ?></span></a>
                <?php } ?>
            <?php } ?>
        </div>
    </div>
