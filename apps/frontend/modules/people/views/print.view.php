...
<html>
<head>
	<title>Друк учасникiв зi списку</title>
	<style type="text/css">
	BODY
	{
		FONT-SIZE: 9pt;
    ...
    <? } ?>
        <? if ( $list ) { ?>
                ...
                <tr>
                    ...
                    <? if($ft['em']){ ?><?=$user_list_sys['email'] ? $user_list_sys['email'] : $user_list_auth['email']?>, <? } ?>
                    <? if($ft['ph']){ ?><?=$user_list_sys['mphone1'] ? $user_list_sys['mphone1'] : ($user_list_sys['phone'] ? $user_list_sys['phone'] : ($user_list_sys['phone2'] ? $user_list_sys['phone2'] : ''))?>, <? } ?>
