...
elseif(request::get_int('ppo')>0)
$receiver_id=request::get_int('ppo');
else $receiver_id=$user['user_id'];
?>
	<form id="compose_form" name="compose_form" class="form mt10" rel="<?=t('Начинайте вводить имя друга')?>..." onsubmit="return false;">
		...
		<table width="100%" class="fs12">
			<tr>
				...
				<?=$group_title?>
                                    <input type="hidden" name="ppo" value="<?=request::get_int('ppo')?>"/>
                                    <input type="hidden" value="0" rel="<?=t('Выберите получателя')?>" style="width: 500px;" name="receiver" />
                                </td>
			</tr>
			<tr>
				<td class="aright"><?=t('Сообщение')?></td>
                                <td><textarea id="body" rel="<?=t('Введите текст сообщения')?>" name="body" style="width: 500px; height:150px;"><?=(request::get('body'))?stripslashes(htmlspecialchars(request::get('body'))):(($message_data['body'])?"\n\n\n".t('Переадресованное сообщение').':'.$message_data['body']:'')?></textarea
                                <?if(request::get_int('group') || request::get_int('ppo')){?>
                                  <br/>  <div class="fs10">* <b>NAME</b> - для зазначення імені в листі</div>
                                    <?}?>
                                </td>
			</tr>
               <? if(!request::get_int('group') && !request::get_int('ppo')){ ?>
                        <tr>
                               <td></td><td style="cursor: pointer; width: 600px;">     
