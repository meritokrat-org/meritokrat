...
class messages_compose_team_action extends messages_controller
{
	public function execute()
	{
		...
		$group_id = request::get_int('team');
		load::model('team/team');
		load::model('team/members');
		$group = team_peer::instance()->get_item($group_id);
		if (!team_members_peer::instance()->allow_edit(session::get_user_id(), $group) && !session::has_credential('admin')) $this->redirect('/');
		...
		if (request::get('submit')) {
			...
			$this->sendteam(request::get_int('team'));
	...
	private function sendteam($group_id)
	{
