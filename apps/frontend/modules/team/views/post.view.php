<?
$ppo = team_peer::instance()->get_item($post_data['team_id']);
?>
<div class="mt10 mr10">
	<h1 class="column_head">
		<div class="left">
			<a href="/team<?= $post_data['team_id'] ?>/<?= $ppo['number'] ?>"><?= $group['title'] ?></a> &rarr;
			<a href="/team/posts?team_id=<?= $post_data['team_id'] ?>"><?= t('Мысли') ?></a>
			&rarr; <?= t('Просмотр') ?>
		</div>
...
<div class="mr10">
	<div class="left acenter mt5" style="width: 80px;">
		...
		<? if (session::is_authenticated()) { ?>
			...
			<div class="fs11 mb10">
				<a href="#add_bookmark" class="b6 <?= ($bkm) ? 'hide' : '' ?>"
				   onclick="Application.bookmarkItem('6','<?= $post_data['user_id'] ?>');return false;"><?= t('Добавить в любимые авторы') ?></a>
				<a href="#del_bookmark" class="b6 <?= ($bkm) ? '' : 'hide' ?>"
				   onclick="Application.unbookmarkItem('6','<?= $post_data['user_id'] ?>');return false;"><?= t('Удалить из любимых авторов') ?></a>
			</div>
		<? } ?>
		...
		<? if (session::is_authenticated() && !$is_blacklisted && !blogs_posts_peer::instance()->has_rated($post_data['id'], session::get_user_id())) { ?>
			<div id="vote_pane">
				<a title="<?= t('Голосовать за') ?>" class="fs11" onclick="ppoController.vote( true );"
				   href="javascript:;"><?= tag_helper::image('common/up.gif') ?></a>
				<a title="<?= t('Голосовать против') ?>" class="fs11" onclick="ppoController.vote( false );"
	...
	<div class="left ml10 mt5" style="width: 645px; margin-left:25px;">
		...
		<div class="fs11 left  mb10">
			...
			<? if (($post_data['user_id'] == session::get_user_id()) || (session::has_credential('moderator')) || team_peer::instance()->is_moderator($post_data['team_id'], session::get_user_id())) { ?>
				<? // <div class="right mr10">Редактировать ?>

				<a id="blogedit<? #(session::get_user_id()==5 or session::get_user_id()==$post_data['user_id']) ? '' : 'blogedit'?>"
				   href="<?= session::get_user_id() == 5 ? '/team/post_edit?id=' . $post_data['id'] : 'javascript:;' ?>"
				   class="ml10"><?= t('Редактировать') ?></a>
...
-->
				...
				<? if (session::is_authenticated()) { ?>
					...
					<a class="ml10 b1 <?= ($bkm) ? 'hide' : '' ?>" href="#add_bookmark"
					   onclick="Application.bookmarkItem('1','<?= $post_data['id'] ?>');return false;"><?= t('В закладки') ?></a>
					<a class="ml10 b1 <?= ($bkm) ? '' : 'hide' ?>" href="#del_bookmark"
					   onclick="Application.unbookmarkItem('1','<?= $post_data['id'] ?>');return false;"><?= t('Удалить из закладок') ?></a>
				<? } ?>
		...
		<div id="additional" style="border-top: 1px solid #EEE;">
			<div class="left mt5">
				...
				<div class="addthis_toolbox addthis_default_style">
					<a onclick="document.getElementById('additional').style.display = 'none';document.getElementById('footer').style.display = 'none';document.getElementById('header').style.display = 'none';document.getElementById('left').style.display = 'none';document.getElementById('comment_form').style.display = 'none';print();"
					   href="javascript:viod(0)"><img src="/static/images/icons/print.gif"
					                                  alt="print"><?= t('напечатать') ?></a>
				</div>
				<!-- AddThis Button END -->
			...
			<? if (session::is_authenticated()) { ?>
				...
				<? if (session::get_user_id() != $post_data['user_id']) { ?>
					<a class="bookmark mb10 ml5 right b1" style="<?= ($bkm) ? 'display:none' : '' ?>"
					   href="#add_bookmark"
					   onclick="Application.bookmarkItem('1','<?= $post_data['id'] ?>');return false;"><b></b><span><?= t('В закладки') ?></span></a>
					<a class="unbkmrk mb10 ml5 right b1" style="<?= ($bkm) ? '' : 'display:none' ?>"
					   href="#del_bookmark"
					   onclick="Application.unbookmarkItem('1','<?= $post_data['id'] ?>');return false;"><b></b><span><?= t('Удалить из закладок') ?></span></a>
				<? } ?>
			<? } ?>
			<div class="clear"></div>
		</div>
		...
		<div class="mt10 mb10" id="comments">
			...
			<? } else { ?>
				<? foreach ($comments as $id) {
					include 'partials/post_comment.php';
				} ?>
			<? } ?>
		</div>
			...
			<? if (!$is_blacklisted) { ?>
				<form class="form_bg" id="comment_form" action="/blogs/comment">
					...
					<div class="ml10 mr10 mb10">
						...
						<input type="button" name="cancel_v" style="display: none;" class="mt5 ml5 mb5 left button_gray"
						       value=" <?= t('Отменить') ?> "/>
