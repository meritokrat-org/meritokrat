...
<div class="mt10 mr10 form_bg">
    <h1 class="column_head"><a
            href="/team<?= $group['id'] ?>/<?= $group['number'] ?>"><?= $group['title'] ?></a> &rarr; <?= t('Редактирование') ?>
    </h1>
    ...
    <div class="tab_pane_gray mb10">
        ...
        <? if (($group['user_id'] == session::get_user_id()) ||
            team_peer::instance()->is_moderator($group['id'], session::get_user_id())
        ) { ?>
    ...
    <!--
		<form id="common_form" action="/team/edit?type=common&submit=1&id=<?= $group['id'] ?>"
				...
				<? if ($group['category'] == 1) { ?>
					<tr>
						...
						<td>
							<select name="ptype">
								<?
        foreach (team_peer::get_ptypes() as $type => $title) { ?>
				...
				<? if (session::has_credential('admin')) { ?>
					...
					<td>
						<select name="category">
							<?
        foreach (team_peer::get_levels() as $level => $title) {
            if ($level == 0) continue; ?>
				...
				<tr id="region_row">
					...
					<td>
						...
						<? if ($group["category"] == 4) { ?>
							<select disabled name="region_id">
								<option selected value="13"><?= t("Киев") ?></option>
							</select>
						<? } else { ?>
							<input type="hidden" name="region_txt" id="region_txt" value="">
							<?= tag_helper::select('region_id', $regions, array('use_values' => false, 'value' => $group['region_id'], 'id' => 'region')) ?>
						<? } ?>
					</td>
				</tr>

				<tr id="scity" class="<?= ($group['category'] > 2) ? 'hidden' : '' ?>">
    ...
    ?>
						<?= tag_helper::select('city_id', $cities, array('use_values' => false, 'value' => $group['city_id'], 'id' => 'city_id')); ?>
					</td>
				</tr>
    ...
    <div id="common_form" class="form">
        <script zz-url="/team/edit/<?= $group["id"] ?>"
    ...
    <? if (session::has_credential('admin')) { ?>
        ...
        <? } ?>
    <? } ?>
    <form id="photo_form" action="/team/edit?type=photo&submit=1&id=<?= $group['id'] ?>"
        ...
        <div class="left acenter" style="width: 250px;">
            <? if ($group['photo_salt']) { ?>
                <?= user_helper::team_photo(user_helper::team_photo_path($group['id'], 'p', $group['photo_salt'])) ?>
            <? } else {
        ...
        <table class="left fs12" style="width: 400px;">
            <tr>
                <td class="cgray fs11" colspan="2">
                    <?= t('Загрузите изображение, символизирующее территорию деятельности Вашей команды (памятник культуры, пейзаж, фото улиц, площадей, любых других объектов, расположенных в пределах территории деятельности команды и являются известными или хотя бы могут помочь отличить Вашу команду от других).') ?>
                    <br/><br/>
    ...
    <? } ?>
    <? if (($group['user_id'] == session::get_user_id()) || team_peer::instance()->is_moderator($group['id'], session::get_user_id())) { ?>
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        <?if(request::get_int('show_teritory')){?>teamController.showTerritory();
        <?}?>
        $("#teritory").click(function () {
            teamController.showTerritory();
            return false;
        });
        $("#glava").click(function () {
            Application.showTeamUsers('glava');
            return false;
        });
        $("#secretar").click(function () {
            Application.showTeamUsers('secretar');
            return false;
        });
