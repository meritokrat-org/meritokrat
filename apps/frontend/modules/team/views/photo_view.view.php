...
<div class="form_bg mr10 fs12 p10 mb10">
	<div class="left">
		<?=t('Фотоальбом')?>
		<a href="/ppo/photo?id=<?=$group['id']?>&album_id=<?=$photo['album_id']?>"><?= $photo['album_id'] ? htmlspecialchars($album['title']) : t('Основной альбом') ?></a>
	</div>
	<a href="/ppo/photo_add?id=<?=$group['id']?>" class="right"><?=t('Загрузить фото')?></a>
	<div class="clear"></div>
</div>
...
<div class="acenter mr10">
	...
	<? if ( ($photo['user_id'] == session::get_user_id()) || ppo_peer::instance()->is_moderator($photo['group_id'], session::get_user_id()) ) { ?>
		<a class="maroon ml10 fs11" href="/ppo/photo_delete?id=<?=$photo['id']?>" onclick="return confirm('<?=t('Вы уверены?')?>');"><?=t('Удалить')?></a>
	<? } ?>
	<br /><br />

	<div class="box_content p10 fs12">
		<? if ( $previous_id ) { ?>
			<a class="left" href="/ppo/photo_view?id=<?=$previous_id?>"><?=t('Предыдущая')?></a>
		<? } ?>
		<? if ( $next_id ) { ?>
			<a class="right" href="/ppo/photo_view?id=<?=$next_id?>"><?=t('Следующая')?></a>
		<? } ?>
...
<? if ( session::is_authenticated()  ) { ?>
	<form class="form_bg" id="comment_form" action="/ppo/photo_comment">
	...
	<form id="comment_reply_form" class="hidden" action="/ppo/photo_comment">
		<input type="hidden" name="photo_id" value="<?=$photo['id']?>"/>
