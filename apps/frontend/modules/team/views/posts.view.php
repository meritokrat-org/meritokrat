<div class="mt10 mr10">
	<h1 class="column_head">
		<div class="left">
			<a href="/team<?= $group['id'] ?>/<?= $group['number'] ?>"><?= $group['title'] ?></a> &rarr;
			<a href="/team/posts?team_id=<?= $group['id'] ?>"><?= t('Мысли') ?></a>
			&rarr; <?= t('Список') ?>
		</div>
		<div class="clear"></div>
	</h1>
</div>
<div class="box_content p5 mb10 fs11 mr10"><a
		href="/team/post_edit?team_id=<?= $group['id'] ?>&add=1"><?= t('Добавить мысль') ?> &rarr;</a></div>
<? if (!$posts) { ?>
...
<? } else { ?>
	<? foreach ($posts as $id) { ?>
		...
		<div class="ml10 mb10 box_content p5" style="width: 730px;">
			<div class="left"
			     style="width:60px"><?= user_helper::photo($post['user_id'], 's', array('class' => 'border1')) ?>
			</div>
			<div class="left ml10" style="width: 525px;">
				...
				<div class="fs11 pb5">
					<div class="left quiet">
						...
						<?= user_helper::full_name($post['user_id'], false, array('class' => 'mr10 fs11'), false) ?> <a
							href="/team/post?team_id=<?= $group['id'] ?>&id=<?= $id ?>"><?= t("Читать дальше") ?> &rarr;</a>
					</div>
				</div>
			</div>
