<div class="mt10 form_bg">
	...
	<h1 class="column_head">
		<a href="/group<?= request::get_int('team_id') ? request::get_int('team_id') : $post_data['team_id'] ?>"><?= $group['title'] ?></a> &rarr;
		<?= $post_data ? t('Редактирование записи') : t('Создание записи') ?></h1>

	<form id="edit_form" name="edit_form" class="form" method="post">
		<input type="hidden" name="team_id"
		       value="<?= $post_data['team_id'] ? $post_data['team_id'] : request::get_int('team_id') ?>"/>
		<? if ($post_data) { ?>
		...
		<table width="100%" class="fs12">
			<? if ($warning == 1) { ?>
				<tr>
					<td class="acenter" colspan=2 style="color:red;"><b>Помилка:</b> Не заповнені всі обов’язкові поля!
					</td>
				</tr>
			<? } ?>
			<tr>
			...
			<tr>
				...
				<td height="350"><textarea rel="<?= t('Введите текст') ?>" name="body"
			...
			<tr>
				...
				<td><input value="1"<?= ($post_data['id'] && db_key::i()->exists($rkey)) ? 'checked' : '' ?>
				           type="checkbox" name="public"/></td>
			</tr>
			<tr>
				<td></td>
				<td>
	...
	<form id="upload_form" class="hide" action="/profile/upload" class="" enctype="multipart/form-data"
