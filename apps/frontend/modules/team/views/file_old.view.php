...
<? if ( (ppo_members_peer::instance()->is_member($group['id'], session::get_user_id()) ) or session::has_credential('admin')) { ?>
	<div class="form_bg mr10 fs12 p10 mb10">
		<div class="left">
			<? if ( $dir_id !== null ) { ?>
				<a href="/ppo/file?id=<?=$group['id']?>"><?=t('Папки сообщества')?></a> &rarr;
		...
		<table width="100%" class="fs12">
			<tr>
                ...
                <div id="add_file" class="<?=request::get('add')==1 ? 'hidden' : ''?>" style="display: <?=request::get_int('add')!=1 ? 'none' : 'block'?>;">
                    <form action="/ppo/file_add?id=<?=$group['id']?>" id="photo_form" class="form" method="post" enctype="multipart/form-data">
		<table width="100%" class="fs12">
			<tr>
			...
			<tr>
                ...
                <div id="add_link" class="hidden" style="display: none;">
                    <form action="/ppo/link_add?id=<?=$group['id']?>" id="link_form" class="form" method="post">
		<table width="100%" class="fs12">
			<tr>
				<td class="aright"><?=t('Название')?></td>
				<td>
...
<? if (ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()) and $dir_id!=0)   { ?>
                        <div class="left ml15"><a class="fs10" onclick="return confirm('Ви впевнені?');" href="/ppo/filedir_delete?id=<?=$dir_id?>">[X]</a></div>
                    <? } ?>
            <div class="clear"></div>
...
<? if (ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()))   { ?>
            ...
            <? foreach ( $files[$dir_id] as $id ) {
                        ...
                        <div class="mt5" style="width:230px"><a href="/download/ppo/<?=$group['id']?>/<?=$file['id']."-".$file['salt'].".".$file['ext']?>"><?=$file['title']?></a><!--br/><span class="fs10"><?=$file['title']?></span-->&nbsp;&nbsp;&nbsp;<?=$file['size']?>
<? if (ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()))   { ?>
                        <a onclick="return confirm('Ви впевнені?');" href="/ppo/file_delete?id=<?=$id?>">[X]</a>
                    <? } ?>
                        <br>
            ...
            <? foreach ( $links[$dir_id] as $id ) {
                       ...
                       <?=$link['title']?></a><!--br/><span class="fs10"><?=$link['title']?></span--><? if (ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()))   { ?>
                        <a onclick="return confirm('Ви впевнені?');" href="/ppo/link_delete?id=<?=$id?>">[X]</a><br>
                    <? } ?>
