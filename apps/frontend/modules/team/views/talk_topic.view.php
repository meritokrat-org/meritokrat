<h2 class="mt10 mr10 column_head">
	<a href="/ppo/talk?id=<?=$group['id']?>"><?=t('Обсуждения')?></a>
	&rarr;
	<?=t('Просмотр темы')?>
</h2>

<h1 class="mt5 mb10 bold cbrown" style="margin-left:65px;">
	...
	<? if ( ( $topic['user_id'] == session::get_user_id() ) || ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()) ) { ?>
		<a href="javascript:;" class="fs11 maroon" onclick="ppoController.deleteTalkTopic(<?=$topic['id']?>);"><?=t('Удалить тему')?></a>
	<? } ?>
</h1>
<?  $counter=0;
    foreach ( $list as $id ) {
    $counter++;
    include 'partials/topic_message.php';
    } ?>
...
<? if ( session::is_authenticated() ) { ?>
	<form class="form_bg mr10 fs12 mb10" action="/ppo/talk_message?id=<?=$topic['id']?>" id="message_form">
