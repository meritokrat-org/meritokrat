...
$dir = team_files_dirs_peer::instance()->get_item($dir_id);
if ($dir_id == 0) $dir['title'] = t('Разное');
$dircounter++; // счетчик "двигателя" по позициям ?>
<? /*  <div id="s_<?=$dir_id?>" class="pointer folder left"> </div> */ ?>

<div id="<?= $dir_id ?>" class="pointer folder_title left mt5 cbrown bold "
     style="width:<?= (750 - $step) ?>px; margin-left: <?= $step ?>px; background-color: #f7f7f7;">
	...
	<div style="margin-top: 1px;" class="right aright mr5">
		<?= count($files[$dir_id]) ?>
		<? if (team_peer::instance()->is_moderator($group['id'], session::get_user_id()) and $dir_id != 0) { ?>
			<? if ($dircounter != 1) { ?><a href="/team/up_dir?dir_id=<?= $dir_id ?>" style="color:#565656;">&#9650;</a>
				<span class="cgray fs11" style="font-weight:normal"><?= $dir['position'] ?></span>
			<? }
			if ($dircounter != $count_dirs) { ?>
			<a href="/team/down_dir?dir_id=<?= $dir_id ?>" style="color:#565656;">
					&#9660;</a><? if ($dircounter == 1) { ?><span class="cgray fs11"
					                                              style="font-weight:normal"><?= $dir['position'] ?></span><? } ?>&nbsp;
			<? } ?>
			<a href="/team/file_edit?dir_id=<?= $dir_id ?>"><img class="ml5" alt="Редагування"
			                                                    src="/static/images/icons/2.2.png"></a>
			<a class="fs12" onclick="return confirm('Ви впевнені?');" href="/team/filedir_delete?id=<?= $dir_id ?>"><img
					class="ml5" alt="Видалити" src="/static/images/icons/3.3.png"></a>
		<? } ?>
	</div>
</div>
<div class="clear"></div>

<div id="files_<?= $dir_id ?>"
     class="<?= ($dir_id == request::get_int('dir_id', null) and $dir_id != 0) ? '' : 'hidden' ?>">
