<div onclick="Application.ShowHide('levels')" style="cursor: pointer;" class="column_head">
    ...
    <div class="right mt5<?= !$cur_category || $cur_list ? ' hide' : '' ?> icoupicon" style="cursor: pointer;"
         id="levels_on"></div>
    <div class="right mt5<?= $cur_category && !$cur_list ? ' hide' : '' ?> icodownt" style="cursor: pointer;"
         id="levels_off"></div>
</div>
<div id="levels" class="p10 box_content<?= !$cur_category || $cur_list ? ' hide' : '' ?>">
    <ul class="mb5">
        <? foreach (team_peer::get_categories() as $id => $title) { ?>
            <li>
                <a href="/team/index?category=<?= $id ?><?= user_helper::createlurl('category') ?>"
                    <?= $cur_category == $id ? 'class="bold"' : '' ?>>
                    <span class="right fs11 bold"
                          style="margin-top: 2px"><?= team_peer::instance()->get_list_count_by_category($id) ?></span>
                    <?= $title ?>
                </a>
            </li>
        <? } ?>
...
<div id="types" class="p10 box_content <?= !$cur_ptype ? 'hide' : '' ?>">
    <ul class="mb5">
        <?
        foreach (team_peer::get_ptypes() as $ptype => $title) { ?>
            <li><a href="/team/index?ptype=<?= $ptype ?><?= user_helper::createlurl('ptype') ?>"
...
<div onclick="Application.ShowHide('lists')" style="cursor: pointer;" class="column_head mt10">
    <div class="left"><?= t('Списки') ?></div>
    <div class="right mt5 <?= !$cur_list ? 'hide' : '' ?> icoupicon" style="cursor: pointer;" id="lists_on"></div>
    <div class="right mt5 <?= $cur_list ? 'hide' : '' ?> icodownt" style="cursor: pointer;" id="lists_off"></div>
</div>
<div id="lists" class="p10 box_content <?= !$cur_list ? 'hide' : '' ?>">
    <ul class="mb5">
        <?
        foreach (lists_peer::instance()->get_list(["in_team" => true]) as $list) {
            $list = lists_peer::instance()->get_item($list);
            ?>
            <li>
                <a href="/team/index?list=<?= $list["id"] ?>"
                   style="<?= $list["id"] == $cur_list ? 'color: #772f23; font-weight: bold; text-decoration: none;' : '' ?>; cursor: pointer; margin: 1px;"><?= $list["title"] ?></a>
            </li>
        <? } ?>
    </ul>
</div>
...
<? if (session::has_credential('admin')) { ?>
    ...
    <div id="statuses" class="p10 box_content <?= !$cur_status ? 'hide' : '' ?>">
        <ul class="mb5">
            <?
            foreach (team_peer::get_statuses() as $status => $title) {
                if ($level != 0) { ?>
                    <li><a href="/team/index?status=<?= $status ?><?= user_helper::createlurl('status') ?>"
...
<div class="p10 box_content <?= !$cur_region ? 'hide' : '' ?>" id="regions">
    ...
    <ul class="mb5 dreg" id="ul_az">
        <? foreach ($az_regions as $region) { ?>
            <li><a href="/team?region=<?= $region['id'] ?><?= user_helper::createlurl('region') ?>"
    ...
    <div class="p10 box_content <?= !$cur_citi ? 'hide' : '' ?>" id="cities">
        ...
        <ul class="mb5 dreg" id="ul_az">
            <? foreach ($az_cities as $region) { ?>
                <li><a href="/team?city=<?= $region['id'] ?><?= user_helper::createlurl('city') ?>"
