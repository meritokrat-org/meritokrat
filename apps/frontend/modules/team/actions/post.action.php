...
class team_post_action extends blogs_controller
{
	public function execute()
	{
		...
		if (!$this->post_data = blogs_posts_peer::instance()->get_item(request::get_int('id'))) {
			$this->redirect('/team' . request::get_int('team_id'));
		}

		$this->group = team_peer::instance()->get_item($this->post_data['team_id']);
		if (($this->group['privacy'] == team_peer::PRIVACY_PRIVATE) && !team_members_peer::instance()->is_member($this->group['id'], session::get_user_id()) && !session::has_credential('admin')) {
			if (session::get_user_id() > 0) $this->redirect('/team' . $this->group['id'] . '/' . $this->group['number'] . '/');
		}
