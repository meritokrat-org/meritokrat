		...
		if ( $text = trim(request::get('text')) )
		{
			...
			if ( user_blacklist_peer::is_banned( $this->post_data['user_id'], session::get_user_id() ) )
			{
				return;
                        ...
                        if(preg_match('/\<div\sclass\=\"quot_box\sfs11\"\>(.*)\<\/div\>(.*)/s', $text, $mch)) {
                                $text = preg_replace('/\<div\sclass\=\"quot_box\sfs11\"\>/', '<QUOTE>', $text);
                                $text = preg_replace('/\<\/div\>/', '</QUOTE>', $text);
                                $text = preg_replace('/\<b\>/', '<BOLD>', $text);
                                $text = preg_replace('/\<\/b\>/', '</BOLD>', $text);
                                $text = preg_replace('/\<span\>/', '<MSG>', $text);
                                $text = preg_replace('/\<\/span\>/', '</MSG>', $text);
                                $text = preg_replace('/\<em\>/', '<DATE>', $text);
                                $text = preg_replace('/\<\/em\>/', '</DATE>', $text);
                                $text = preg_replace('/\<br\>/', '', $text);
                        }
                        if ( $upd_id = request::get_int('upd_id') )
                        {
                            $children = blogs_comments_peer::instance()->get_list(array('parent_id'=>$upd_id));
                            $comment = blogs_comments_peer::instance()->get_item($upd_id);
                            
                            if(session::has_credential('admin') || session::has_credential('moderator') || $comment['user_id']==session::get_user_id()) {
                                if(!empty($children) && !session::has_credential('admin')) 
                                        die(json_encode(array('error'=>t('Вы не можете редактировать комментарий на который ответили'))));
                                ...
                                die(json_encode(array('text'=>nl2br(stripslashes($text)))));
                            } else {
                                die(json_encode(array('error'=>t('Вы не имеете прав для редактирования этого комментария'))));
                            }
                        }
                        else
                        {
                            if($pId = request::get_int('parent_id')) {
                                $check = blogs_comments_peer::instance()->get_item($pId);
                                if(!$check)
                                    die(json_encode(array('error'=>t('Комментарий, на который Вы питаетесь ответить, был удален или не существует'))));
                            }
                            ...
                            if(request::get('neg_msg')==1) {
                                ...
                                //if (session::has_credential('admin')) echo 'negative_blog_comment:'.$this->id;
                            }
