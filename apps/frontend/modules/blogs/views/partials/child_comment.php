	...
	<div style="margin-left: 60px;">
		...
		<!--div class="right">
			<? if ( session::is_authenticated() && !blogs_comments_peer::instance()->has_rated($child_comment['id'], session::get_user_id()) ) { ?>
				<span>
					<a href="javascript:;" onclick="blogsController.rateComment(this, <?=$child_comment['id']?>, true);"><?=tag_helper::image('common/up.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
					<a href="javascript:;" onclick="blogsController.rateComment(this, <?=$child_comment['id']?>, false);"><?=tag_helper::image('common/down.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
				</span>
			<? } ?>
		...
		<br />
		<div class="combody mt10 comment_hdata<?=$child_comment['id']?> fs12 <?=$do_hide_comment ? 'hidden' : ''?>">
                        <?=user_helper::get_links($child_comment['text'])?>
                </div>
                <div class="comment_hdata<?=$child_comment['id']?>">
		<? if ( session::is_authenticated() && !$ajaxaction ) { ?>
                    <? if($level[$child_id]<=3){ ?>
                        <a href="javascript:;" rel="<?=$child_comment['id']?>" class="dotted comment_reply"><?=t('Ответить')?></a>
                    <? } ?>
                        <a href="#comment_form" onClick="func();" rel="<?=$comment['id']?>" class="dotted comment_quote_reply <?=($level[$child_id]<=3)?'ml10':''?>"><?=t('Цитувати')?></a>
                <? } ?>
		<? if ( session::has_credential('moderator') ||
			...
			<a href="javascript:;" rel="<?=$child_comment['id']?>" class="dotted comment_update <?=(!$ajaxaction)?'ml10':''?>" onClick="Application.<?=($child_comment['user_id']==session::get_user_id()) ? 'initComUpdUser' : 'initComUpd'?>('<?=$child_comment['id']?>')"><?=t('Редактировать')?></a>
                        <a href="javascript:;" onclick="Application.delCom('<?=$child_comment['id']?>','blogs/delete_comment',<?=($child_comment['user_id']==session::get_user_id())?1:0?>)" class="dotted ml10"><?=t('Удалить')?></a>
		<? } ?>
                </div>
	</div>
	<div class="clear"></div>
