...
<? } else { ?>
	<form rel="<?= t('Выберите друзей из списка') ?>" id="share_form" action="/invites" method="post"
	      onsubmit="return Application.doInvite();">

		<input type="hidden" name="stype" value=""/>

		<div class="m10 fs11 aleft" style="width: 550px;">
			<? if ($item_type != 4) { ?>
				<div id="user" class="tab fs14 p5 mr5 quiet left border1">
					<a style="color: black; text-decoration: none;"
					   onclick="$('#search, #friend_selector').hide();$('#user_pager, #users_selector').show();$('#result').show();$('#friend_tab').addClass('unselected');$('#user_tab').removeClass('unselected');"
					   href="javascript:;"><?= t('Все') ?></a>
				</div>
				<div id="friend" class="tab fs14 p5 mr5 quiet unselected left border1">
					<a style="color: black; text-decoration: none;"
					   onclick="$('#friend_tab').removeClass('unselected');$('#user_tab').addClass('unselected');"
					   href="javascript:;"><?= t('Друзья') ?></a>
				</div>


				<? if (session::has_credential('admin')) { // && ($item_type==1 || $item_type==3)?>
					<div id="regions" class="tab fs14 p5 mr5 quiet unselected left border1">
						<a style="color: black; text-decoration: none;" onclick=""
						   href="javascript:;">*<?= t('Регион') ?></a>
					</div>
					<div id="groups" class="tab fs14 p5 mr5 quiet unselected left border1">
						<a style="color: black; text-decoration: none;" onclick=""
						   href="javascript:;">*<?= t('Сообщества') ?></a>
					</div>
					<div id="status" class="tab fs14 p5 mr5 quiet unselected left border1">
						<a style="color: black; text-decoration: none;" onclick=""
						   href="javascript:;">*<?= t('Статус') ?></a>
					</div>
					<div id="functions" class="tab fs14 p5 mr5 quiet unselected left border1">
						<a style="color: black; text-decoration: none;" onclick=""
						   href="javascript:;">*<?= t('Функции') ?></a>
					</div>
					<div id="lists" class="tab fs14 p5 mr5 quiet unselected left border1">
						<a style="color: black; text-decoration: none;" onclick=""
						   href="javascript:;">*<?= t('Списки') ?></a>
					</div>
				<? } ?>
			<? } ?>
			<? if (session::has_credential('admin') && $item_type == 2) { ?>
				<div id="invited" class="tab fs14 p5 mr5 quiet unselected left border1">
					<a style="color: black; text-decoration: none;" onclick=""
					   href="javascript:;">*<?= t('Приглашены') ?></a>
				</div>
			<? } ?>
			...
			<? if ($data['type'] == 1 && groups_peer::instance()->is_moderator($data['content_id'], session::get_user_id()) && $item_type == 1) { ?>
				<div class="mb5 quiet right">
					<input type="checkbox" id="select_all_friends" name="all" value="<?= $data['id'] ?>"/>
			...
			<div class="friend_selector" id="user_selector">
				...
				<div class="clear"></div>
				<div id="user_pager"
				     class="mt10 mb5 ml5 left pager"><?= pager_helper::get_full_ajax($userpager, null, 12, 18, $q) ?></div>
				<div class="clear pb10"></div>
			</div>

			<div id="friend_selector" class="friend_selector" style="height:400px;overflow:auto;display:none">
				<? foreach ($friends as $friend_id) { ?>
					<input type="checkbox" class="friend_check hidden" name="fs[]" id="friend_check_<?= $friend_id ?>"
					       value="<?= $friend_id ?>">
					<div id="friend_<?= $friend_id ?>" rel="<?= $friend_id ?>" class="item friend"
					     onclick="Application.thisfriendSelect(this);" style="height:50px;">
						<?= user_helper::photo($friend_id, 's', array('class' => 'left', 'width' => '40'), false) ?>
						<div style="margin-left: 50px;">
							<?= user_helper::full_name($friend_id, true, array('class' => 'dotted', 'target' => '_blank'), false) ?>
							<?= (is_array($invited) && in_array($friend_id, $invited)) ? '<br/><span style="color:green;">' . t('Уже приглашен') . '</span>' : '' ?>
						</div>
					</div>
				<? } ?>
				<div class="clear"></div>
			...
			<? if (session::has_credential('admin') && $item_type == 2) { ?>
				...
				<div id="invited_selector" class="friend_selector" style="height:375px;overflow:auto;display:none">
					<? foreach ($invited as $invited_id) { ?>
						<div id="friend_<?= $invited_id ?>" rel="<?= $invited_id ?>" class="item friend"
						     style="height:50px;width:230px;">
							<?= user_helper::photo($invited_id, 's', array('class' => 'left', 'width' => '40'), false) ?>
							<div style="margin-left: 50px;">
								<?= user_helper::full_name($invited_id, true, array('class' => 'dotted', 'target' => '_blank'), false) ?>
								<a class="right reject"><?= t('Отменить') ?></a>
								<br/>
								<?= t('Пригласил') ?>
								: <?= user_helper::get_inviters($invited_id, '2', $data['id'], true) ?>
							</div>
						</div>
					<? } ?>
					<div class="clear pb10"></div>
					<div
						class="mt10 mb5 ml5 left pager"><?= page_helper::get_full_ajax($invpager, null, 12, 18, $q, '/invites/invited') ?></div>
				</div>
			<? } ?>
			...
			<? if (session::has_credential('admin')) { // && ($item_type==1 || $item_type==3)?>
				...
				<div id="regions_selector" class="friend_selector" style="height:400px;overflow:auto;display:none">
					<? foreach ($regions as $k => $v) { ?>
						<input type="checkbox" class="friend_check hidden" name="fs[]" id="friend_check_<?= $k ?>"
						       value="<?= $k ?>">
						<div id="friend_<?= $k ?>" rel="<?= $k ?>" class="item friend"
						     onclick="Application.thisfriendSelect(this);" style="height:50px;">
				...
				<div id="groups_selector" class="friend_selector" style="height:400px;overflow:auto;display:none">
					<? foreach ($groups as $gid) { ?>
						<input type="checkbox" class="friend_check hidden" name="fs[]" id="friend_check_<?= $gid ?>"
						       value="<?= $gid ?>">
						<div id="friend_<?= $gid ?>" rel="<?= $gid ?>" class="item friend"
						     onclick="Application.thisfriendSelect(this);" style="height:50px;">
							<?= group_helper::photo($gid, 'sm', false, array('class' => 'left', 'width' => '40')) ?>
							<div style="margin-left: 50px;"><a
									class="dotted"><?= group_helper::title($gid, false) ?></a></div>
						</div>
					<? } ?>
					<div class="clear pb10"></div>
				</div>
				...
				<div id="functions_selector" class="friend_selector" style="height:400px;overflow:auto;display:none">
					<? foreach (user_auth_peer::get_functions() as $function_id => $function_title) { ?>
						<input type="checkbox" class="friend_check hidden" name="fs[]"
						       id="friend_check_<?= $function_id ?>" value="<?= $function_id ?>">
						<div id="friend_<?= $function_id ?>" rel="<?= $function_id ?>" class="item friend"
						     onclick="Application.thisfriendSelect(this);" style="height:50px;">
							<? //user_helper::photo(0, 's', array('class' => 'left', 'width' => '40'), false)?>
				...
				<div id="lists_selector" class="friend_selector" style="height:400px;overflow:auto;display:none">
					<? foreach ($lists as $list_id) { ?>
						...
						<div id="friend_<?= $list_id ?>" rel="<?= $list_id ?>" class="item friend"
						     onclick="Application.thisfriendSelect(this);" style="height:50px;">
							<? //user_helper::photo(0, 's', array('class' => 'left', 'width' => '40'), false)?>
							<div style="margin-left: 50px;"><a class="dotted"><?= $item['title'] ?></a></div>
						</div>
					<? } ?>
					<div class="clear pb10"></div>
				</div>
			...
			<div class="left">
				<div id="friends_checker">
					...
					<textarea name="message" style="height: 50px; width: 200px;"><?= $message ?> <?= $name ?></textarea>
				<? } ?>
				<div class="mt10">
					<input type="submit" class="button" value=" <?= t('Пригласить') ?> ">
					<input type="button" class="button_gray" onclick="Application.invitedFriends=[];Popup.close();"
					       value=" <?= t('Отмена') ?> ">
					<? if (session::has_credential('admin') && in_array($item_type, [2, 4, 5, 6])) { ?>
						<input type="button" name="jusers" id="jusers" class="button" value=" Приєднати "
							<?
							$doto = "Ppo";
							if($item_type == 2)
								$doto = "Group";
							...
							elseif($item_type == 6)
								$doto = "Projects";
							?>
						       onclick="return Application.doto<?=$doto?>();">
						<!--a id="join_users" class="button ml15" style="padding:2px;"> Приєднати </a-->
					<? } elseif (session::has_credential('admin') && $item_type = 1 && $data['start'] < time()) { ?>
						<input type="button" class="button" value=" <?= t('Был') ?> "
						       onclick="return Application.dotoEvent(1);">
						<input type="button" class="button_gray" value=" <?= t('Нет') ?> "
						       onclick="return Application.dotoEvent(2);">
					<? } ?>
				</div>
			</div>
