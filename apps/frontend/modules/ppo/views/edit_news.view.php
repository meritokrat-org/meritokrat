...
<script type="text/javascript">
	...
	<h1 class="column_head"><?=t('Редактирование новости')?></h1>
        <? include 'partials/tabs.php' ?>
<form id="edit_form" method="post" class="form form_bg mt10">


	<table width="100%" class="fs12">
		<tr>
			<td width="18%" class="aright"><?=t('Заголовок')?></td>
                        <td><input value="<?=$post_data['title']?>" rel="<?=t('Введите заголовок новости')?>" style="width: 350px;" name="title" type="text" class="text" /></td>
		</tr>
		
                <tr>
                        <td class="aright"><?=t('Текст')?></td>
                        <td><textarea name="text" style="width: 350px;height: 250px;"><?=$post_data['text']?></textarea></td>
                </tr>
                        <tr>
...
tinyMCE.init({
   ...
   <? } ?>
	...
	content_css: '/static/css/typography.css'
        ...
        <form id="upload_form" class="hide form" action="/profile/upload" enctype="multipart/form-data" style="position:absolute;">
    ...
    <form method="post" id="photo_form" class="form_bg mt10 form hidden" enctype="multipart/form-data">
    		<table width="100%" class="fs12"><tr>
               ...
               echo image_helper::photo($post_data['photo'], 'p', 'groupnews', array('class' => 'border1', 'id' => 'photo'));
        ?>
