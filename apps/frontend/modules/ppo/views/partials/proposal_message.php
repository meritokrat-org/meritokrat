<? $topic_message = ppo_proposal_messages_peer::instance()->get_item($id) ?>
<div class="mb10 comment_bg mr10" id="talk_message<?=$id?>">
	...
	<div class="left ml10" style="width: 680px;">
		...
		<div class="right">
			<?  if ( session::is_authenticated() && !ppo_proposal_messages_peer::instance()->has_rated($topic_message['id'], session::get_user_id()) ) { ?>
				<span>
					<a href="javascript:;" onclick="ppoController.rateProposalMessage(this, <?=$topic_message['id']?>, true);"><?=tag_helper::image('common/up.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
					<a href="javascript:;" onclick="ppoController.rateProposalMessage(this, <?=$topic_message['id']?>, false);"><?=tag_helper::image('common/down.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
				</span>
			<? }  ?>
		...
		<? if ( ( $topic_message['user_id'] == session::get_user_id() ) || ppo_peer::instance()->is_moderator($group['id'], session::get_user_id()) ) { ?>
			<div class="fs10 mt10">
				<a href="javascript:;" onclick="ppoController.deleteProposalMessage(<?=$id?>);" class="maroon dotted"><?=t('Удалить')?></a>
                                <a href="/ppo/proposal_message_edit?id=<?=$id?><?=$counter==1 ? '&tinymce=1' : ''?>" class="maroon dotted"><?=t('Редактировать')?></a>
			</div>
		<? } ?>
	</div>
	<div class="clear"></div>
