...
<div id="types" class="p10 box_content <?= !$cur_ptype ? 'hide' : '' ?>">
    <ul class="mb5">
        <?php
        foreach (ppo_peer::get_ptypes() as $ptype => $title) { ?>
            <li><a href="/ppo/index?ptype=<?= $ptype ?><?= user_helper::createlurl('ptype') ?>"
...
<?php if (session::has_credential('admin')) { ?>
    ...
    <div id="statuses" class="p10 box_content <?= !$cur_status ? 'hide' : '' ?>">
        <ul class="mb5">
            <?php
            foreach (ppo_peer::get_statuses() as $status => $title) {
                if (0 !== $level) { ?>
                    <li><a href="/ppo/index?status=<?= $status ?><?= user_helper::createlurl('status') ?>"
...
<div class="p10 box_content <?= !$cur_region ? 'hide' : '' ?>" id="regions">
    ...
    <ul class="mb5 dreg" id="ul_az">
        <?php foreach ($az_regions as $region) { ?>
            <li><a href="/ppo?region=<?= $region['id'] ?><?= user_helper::createlurl('region') ?>"
...
<? if (request::get_int('region')) { ?>
    ...
    <div class="p10 box_content <?= !$cur_citi ? 'hide' : '' ?>" id="cities">
        ...
        <ul class="mb5 dreg" id="ul_az">
            <?php foreach ($az_cities as $region) { ?>
                <li><a href="/ppo?city=<?= $region['id'] ?><?= user_helper::createlurl('city') ?>"
        ...
        <ul class="mb5 dreg hide" id="ul_unrate">
            <?php foreach ($rate_cities as $region_count => $region) { ?>
                <li><a href="/ppo?city=<?= $region['id'] ?><?= user_helper::createlurl('city') ?>"
                       style="margin: 1px;"><?= $region['title'] ?></a>
                    <div class="cbrown right fs11 bold"><?= $region['count'] ?></div>
                </li>
