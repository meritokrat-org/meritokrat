<? foreach ( $users as $user_id ) { ?>
        ...
        <div id="friend_<?=$user_id?>" rel="<?=$user_id?>" class="item friend" onclick="<?=request::get_int('more_select')?'Application.thisfriendSelect(this);':'Application.thisOneUserSelect(this);'?>"  style="height:50px;">
                <?=user_helper::photo($user_id, 's', array('class' => 'left', 'width' => '40'), false)?>
                <div style="margin-left: 50px;">
                    <a class="dotted" target="_blank" href="/profile-<?=$user_id?>"><?=user_helper::full_name($user_id, false)?></a>
...
$pager = pager_helper::get_full_ajax($pager,$page,12,16,$q,$srch);?>
    <?$pager=str_replace("Srch", "Ppo",$pager);?>
    <?$pager=str_replace(")\"", ",'".request::get_int('ppo_id')."')\"",$pager);?>
