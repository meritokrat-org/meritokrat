<h1 class="column_head mt10 mr10">
	<a href="/ppo<?=$group['id']?>/<?=$group['number']?>"><?=stripslashes(htmlspecialchars($group['title']))?></a>
	&rarr;
	<?=t('Новости')?>
</h1>
...
<div class="mr10">
	...
	<? foreach ( $list as $id ) { ?>
		<? $news_item = ppo_news_peer::instance()->get_item($id); ?>

			<div class="left  p5 fs12 bold box_content " id="news_title_<?=$id?>" class="fs14"><?=stripslashes(htmlspecialchars($news_item['title']))?> </div>
    ...
    <a href="javascript:;" onclick="if ( confirm('<?=t('Удалить новость?')?>') ) ppoController.deleteNews(<?=$news_item['id']?>,'/ppo/news?id=<?=$group['id']?>');" class="dotted ml10"><?=t('Удалить')?></a>
    <a class="ml10" href="/ppo/edit_news?id=<?=$news_item['id']?>" id="newsedit"><?=t('Редактировать')?></a>
<?}?>
		</div>
		<div class="fs12" id="news_body_<?=$id?>">
			...
			<p><?=stripslashes($news_item['text'])?></p>
                        <a class="right" href="/ppo/newsread?id=<?=$news_item['id']?>"><?=t('Читать дальше')?> &rarr;</a>
		</div>
                        <div class="clear"></div>
	<? } ?>
...
<form id="edit_news_form" class="hidden" action="/ppo/save_news">
