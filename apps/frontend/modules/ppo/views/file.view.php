...
<h1 class="mt10 mr10 column_head">
	<a href="/ppo<?=$group['id']?>/<?=$group['number']?>"><?=htmlspecialchars($group['title'])?></a> &rarr; <?=t('Файлы')?>
</h1>


<?   $max=db::get_row("SELECT max(position) FROM ppo_files_dirs WHERE group_id=:group_id",array('group_id'=>$group['id']));    ?>
	<div class="form_bg mr10 fs12 p10 mb10">
		...
		<div class="left" style="color:#333333; text-align: justify;">
            ...
            <div class="right">
            <?  if (ppo_peer::instance()->is_moderator($group['id'], session::get_user_id())) { ?><a onclick="$('#add_photoalbum').show(50);" href="javascript:;"><?=t('Добавить папку')?></a>&nbsp;|&nbsp;<? } ?><?  if (ppo_members_peer::instance()->is_member($group['id'], session::get_user_id()) || session::has_credential('admin')) { ?><a onclick="$('#add_stuff').show(50);$('#add_file').show(50);" href="javascript:;" class="right">&nbsp;<?=t('Добавить материал')?></a><? } ?>
            </div>
...
<? } ?>
<?  if (ppo_members_peer::instance()->is_member($group['id'], session::get_user_id()) || session::has_credential('admin')) { ?>
		...
		<table width="100%" class="fs12">
			<tr>
                ...
                <div id="add_file" class="<?=request::get('add')==1 ? 'hidden' : ''?>" style="display: <?=request::get_int('add')!=1 ? 'none' : 'block'?>;">
                    <form action="/ppo/file_add" id="photo_form" class="form" method="post" enctype="multipart/form-data">
			<input type="hidden" value="<?=$group['id']?>" name="id" id="id">
		<table width="100%" class="fs12">
			<tr>
			...
			<tr>
                ...
                <div id="add_link" class="hidden" style="display: none;">
                    <form action="/ppo/file_add" id="link_form" class="form" method="post">
			<input type="hidden" value="<?=$group['id']?>" name="id" id="id">
		<table width="100%" class="fs12">
			<tr>
				<td class="aright"><?=t('Название')?></td>
...
<? } ?>
  ...
  <?  }   ?>
            ...
            <? foreach ( ppo_files_peer::instance()->get_list(array('dir_id'=>0,'group_id'=>$group['id']),array(),array('position ASC')) as $id ) {
                    $counter++;
