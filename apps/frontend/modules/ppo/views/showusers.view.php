...
<script>
    ...
    function search() {
        ...
        $.get('/ppo/share_user', {
                    q: zelenoye,
                    ppo_id: $('#ppo_id').val(),
                    more_select: <?=request::get_int('more_select')?>,
                },
                function (data) {
    ...
    $('#share_form').submit(function () {
        <?php if(1 === request::get_int('more_select')){?>
        var uchval = '';
        ...
        <?php if(1 === request::get_int('multi')){?>
        $('<tr id="member' + thisid + '"><td></td><td><input type="hidden" value="' + thisid + '" name="<?=request::get_string('field_name', 'function4')?>[]"/>' + name + ' - <a class="one_delete_function" rel="' + thisid + '" href="javascript:;"><?= t('Удалить') ?></a></td></tr>').insertBefore('.' + id);
        <?php } else { ?>
        $('#' + id).val(name);
        $('#' + id + 'div').html(name);
        <?php } ?>
            ...
            <div id="result">
                ...
                <? foreach ($users as $user_id) { ?>
                    ...
                    <div id="friend_<?= $user_id ?>" rel="<?= $user_id ?>" class="item friend"
                         onclick="Application.thisOneUserSelect(this);" style="height:50px;">
                        <?= user_helper::photo($user_id, 's', ['class' => 'left', 'width' => '40'], false) ?>
                        <div style="margin-left: 50px;">
                            <a class="dotted"><?= user_helper::full_name($user_id, false, [], false) ?></a>
                        </div>
                    </div>
                <? } ?>
