...
class ppo_view_action extends ppo_controller
{
    ...
    public function execute()
    {
        ...
        $this->inv_owners = ppo_members_peer::instance()->get_members($this->group['id'], false, $this->group);
        if ($this->inv_owners) {
        ...
        if (!$this->group || (!session::has_credential('admin') and $this->group['active'] != 1)) {
            $this->redirect('/ppo');
        }
        client_helper::set_title(stripslashes(htmlspecialchars($this->group['title'])).' | '.conf::get('project_name'));

        $user_desktop         = db::get_row("SELECT * FROM user_desktop WHERE user_id=:user_id", ['user_id' => $this->user['id']]);
        $this->user_functions = (array) explode(',', str_replace(['{', '}'], ['', ''], $user_desktop['functions']));

        $this->moderators = ppo_peer::instance()->get_moderators($this->group['id'], $this->group);
        ...
        $this->is_member = ppo_members_peer::instance()->is_member($this->group['id'], session::get_user_id());
        if (!$this->is_member) {
        ...
        if (($this->group['privacy'] == ppo_peer::PRIVACY_PRIVATE) && !$this->is_member && !session::has_credential('admin')) {
        ...
        $this->files = ppo_files_peer::instance()->get_by_group($this->group['id']);
        ...
        $this->news        = ppo_news_peer::instance()->get_by_group($this->group['id']);
        $this->members     = ppo_members_peer::instance()->get_members($this->group['id'], false, $this->group);
        ...
        if ($this->group['category'] > 1) {
            $this->children_ppo = ppo_peer::instance()->get_children($this->group, $this->group['category']);
            if ($this->group['category'] > 2) {
                $this->children_mpo = ppo_peer::instance()->get_children($this->group, 3, 2);
            }
        }

        load::model('eventreport/eventreport');
        $this->is_leader = ppo_members_peer::instance()->is_leader(session::get_user_id());
