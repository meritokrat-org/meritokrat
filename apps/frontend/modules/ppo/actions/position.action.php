...
class ppo_position_action extends ppo_controller
{
	public function execute()
	{
		...
		$this->list = ppo_positions_peer::instance()->get_by_group($this->group['id'], $sort);

		$this->pager = pager_helper::get_pager($this->list, request::get_int('page'), 10);
		$this->list = $this->pager->get_list();

		client_helper::set_title( t('Позиция МПУ') . ' | ' . $this->group['title'] );
	}
}