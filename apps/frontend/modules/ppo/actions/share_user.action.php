...
class ppo_share_user_action extends ppo_controller
{
    public function execute()
    {
        ...
        if ($key = request::get('q')) {
            if (strlen($key) > 2) {
                $this->users = user_data_peer::instance()->get_by_name_ppo($key, $where);
            }
        } else {
            $this->users = db::get_cols(sprintf('SELECT id FROM user_auth %s', $where));
        }

        if (request::get_int('ppo_id') > 0 && $this->users) {
            $this->group = ppo_peer::instance()->get_item(request::get_int('ppo_id'));
            $ppo_users = ppo_members_peer::instance()->get_members(request::get_int('ppo_id'), false, $this->group);
