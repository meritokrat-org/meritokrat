...
<div class="mr10">
	...
	<? foreach ($list as $id) { ?>
		<? $news_item = reform_news_peer::instance()->get_item($id); ?>
		...
		<div id="news_head_<?= $id ?>" class="mb5 quiet fs11 box_content p5">&nbsp;&nbsp;&nbsp;<br><br>
			...
			<? if (reform_peer::instance()->is_moderator($group['id'], session::get_user_id())) { ?>
				<a href="javascript:;"
				   onclick="if ( confirm('<?= t('Удалить новость?') ?>') ) reformController.deleteNews(<?= $news_item['id'] ?>,'/projects/news?id=<?= $group['id'] ?>');"
				   class="dotted ml10"><?= t('Удалить') ?></a>
				<a class="ml10" href="/ppo/edit_news?id=<?= $news_item['id'] ?>"
		...
		<div class="fs12" id="news_body_<?= $id ?>">
			...
			<a class="right" href="/projects/newsread?id=<?= $news_item['id'] ?>"><?= t('Читать дальше') ?> &rarr;</a>
		</div>
		<div class="clear"></div>
	<? } ?>
...
<form id="edit_news_form" class="hidden" action="/projects/save_news">
