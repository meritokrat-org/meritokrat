				...
				<tr>
					<td>
						...
						foreach ($inventory_type as $inv_id) {
							$ids = db::get_cols("SELECT id FROM party_inventory WHERE inventory_type=:itype AND reform_id IN (" . implode(",", $inv_owners) . ")", array('itype' => $inv_id));
							$sum = db::get_scalar("SELECT SUM(inventory_count) FROM party_inventory WHERE inventory_type=:itype AND reform_id IN (" . implode(",", $inv_owners) . ")", array('itype' => $inv_id));
							$total_cost = db::get_scalar("SELECT SUM(inventory_count*cost) FROM party_inventory WHERE inventory_type=:itype AND reform_id IN (" . implode(",", $inv_owners) . ")", array('itype' => $inv_id));
							$total += $sum;
							?>
...
//                        exit;
										...
										<tr style="background: #ffffff;">
											...
											<td>
												<input type="text" name="inventory_count[]" class="text"
												       value="<?= $data['inventory_count'] ?>">
											</td>
											<td>
												<input type="text" name="inventory_cost[]" class="text"
												       style="width: 50px;" value="<?= $data['cost'] ?>">
											</td>
											<td>
												<input type="text" name="cost_sum[]" disabled class="text"
												       style="width: 50px;;"
												       value="<?= $data['cost'] * $data['inventory_count'] ?>">
											</td>
											<td>
												<input type="text" name="inventory_date[]" id="date_<?= $c; ?>"
												       class="text date_pic"
												       value="<?= date('d.m.Y', $data['date_ts']) ?>  ">
											</td>
											<td>
				...
				<tr>
					<td>
						...
						<input onclick="$('.inner_table').slideToggle(1)" type="button" name="cancel" class="button"
						       value="<?= t('Детальная информация') ?> ">
	...
	<? } ?>
	<input style="margin-left: 5px;" onclick="$('#add_inventory_form').slideToggle(300)" type="button" name="cancel"
	       class="button" value="+ <?= t('Добавить') ?> ">
	...
	function delete_item(id) {
		$.ajax({
			...
			success: function (resp) {
				data = eval("(" + resp + ")");
				if (data.success == 1) $('.success').fadeIn(300, function () {
					$(this).fadeOut(2000, function () {
						url = window.location;
						window.location = url + '&tab=inventory'
					});
				});
