...
class ppo_talk_create_action extends ppo_controller
{
	...
	public function execute()
	{
		...
		if ( ( $this->group['privacy'] == ppo_peer::PRIVACY_PRIVATE ) && !ppo_members_peer::instance()->is_member($this->group['id'], session::get_user_id())  &&  !ppo_peer::instance()->is_moderator($this->group['id'], session::get_user_id()))
		{
		...
		{
			$id = ppo_topics_peer::instance()->insert(array(
		...
		$this->redirect('/ppo/talk_topic?id='.$id);
	}
}