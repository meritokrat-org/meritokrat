...
class reform_addmembers_action extends reform_controller
{
	public function execute()
	{
		...
		if (count($this->friends) > 0) {
			foreach ($this->friends as $friend_id) {
				...
				if ( ! reform_members_peer::instance()->is_member($this->id, $friend_id)) {
					reform_members_peer::instance()->add($this->id, $friend_id, 2);
					reform_peer::instance()->update_rate($this->id, 1, $friend_id);
					reform_members_history_peer::instance()->set_member_history($this->id, $friend_id, time());
				}
