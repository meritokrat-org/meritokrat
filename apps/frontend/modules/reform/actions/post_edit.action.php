...
class ppo_post_edit_action extends blogs_controller
{
	...
	public function execute()
	{
		...
		client_helper::set_title( t('Мысли') .' '. t('Партийные организации') . ' | ' . conf::get('project_name') );
		...
		if ( request::get_int('id') )
		{
                        ...
                        if ( ( $this->post_data['user_id'] != session::get_user_id() ) && !session::has_credential('moderator') && !ppo_peer::instance()->is_moderator($this->post_data['group_id'], session::get_user_id()))
			{
                ...
                request::get_int('group_id') ? $group_id=request::get_int('group_id') : $group_id=$this->post_data['ppo_id'];
                    
                //if (session::get_user_id()==996) var_dump(ppo_members_peer::instance()->is_member(request::get_int('group_id'), session::get_user_id() )).var_dump(ppo_peer::instance()->is_moderator(request::get_int('group_id'), session::get_user_id()));  
                if (ppo_peer::instance()->is_moderator($group_id, session::get_user_id()) || (user_auth_peer::instance()->get_rights(session::get_user_id(),10) && ppo_members_peer::instance()->is_member($group_id, session::get_user_id() )) ) { 
                        ...
                        if ( request::get('submit'))
                        {
                            if (trim(request::get('body')) && trim(request::get('title')) && trim(request::get('group_id')))
                            {
                                ...
                                $data = array(
                                        ...
                                        'ppo_id' => request::get_int('group_id')
                                );
                            ...
                            $this->rkey="public_ppo_post_".$post_id;
                            ...
                            db_key::i()->set($this->rkey, 1);
                                ...
                                $this->redirect('/ppo/post?group_id='.$this->group['id'].'&id='.$post_id);
                            }
                            else $this->warning=1;
                        }else $this->rkey="public_ppo_post_".request::get_int('id');
