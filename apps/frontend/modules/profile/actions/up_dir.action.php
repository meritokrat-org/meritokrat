<?
class profile_up_dir_action extends frontend_controller
{
		...
		if ( request::get('dir_id') )
			...
			$dir_id = request::get_int('dir_id');
                        $dirs = library_files_dirs_peer::instance()->get_list(
                                array("object_id"=>$this->user_id,"type"=>1), array(), 
                                array('position ASC'));
...
//информация о папке выше
			...
			$dir = db::get_row( $sql, $bind, null );
                        ...
                        if ($position>0)
                        {
                        db::exec("UPDATE files_dirs SET position=position+1 WHERE position=:position AND object_id=:object_id AND type=1", 
