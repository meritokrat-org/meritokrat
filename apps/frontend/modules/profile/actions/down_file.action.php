<?
class profile_down_file_action extends frontend_controller
{
	public function execute()
	{
		...
		if ( request::get('id') )
			...
			$file = db::get_row( $sql, $bind, null );
                        ...
                        if ($position>0)
                        {
                        db::exec("UPDATE files SET position=position-1 WHERE position=:position AND dir_id=:dir_id
                            AND object_id=:object_id AND type=1", array('position'=>$position,'dir_id'=>$dir_id,"object_id"=>$this->user_id));
