var teamController = new function () {
        ...
        var form = new Form('add_form', {
            ...
            success: function (response) {
                ...
                if (response.success > 0)
                    document.location = '/team/create?success=' + response.id;
                else
                    document.location = '/team/edit?id=' + response.id + '&tab=photo';
            },
            wait_panel: 'wait_panel'
        });
    };

    this.showTerritory = function (nocontrols, team_id) {
        Popup.show();
        $.post('/team/show_territory', {
    ...
    this.editAction = function () {
        ...
        //                 $('.success').fadeOut(1500);
        //             });
        //             document.location = '/team/edit?id=' + response.id;
        //         },
        ...
        var leadershipForm = new Form(
            'leadership_form',
            {
                ...
                success: function (response) {
                    ...
                    document.location = '/team/edit?id=' + response.id + '&tab=leadership';
                },
    ...
    //			success: function( response )
    //			{
    ///				document.location = '/team/talk_topic?id=' + response.id;
    //			}
    //		}
    //	);
    //};
    ...
    this.positionAction = function () {
        var form = new Form(
            'topic_form',
            {
                ...
                success: function (response) {
                    document.location = '/team/position_topic?id=' + response.id;
                }
            }
        );
    };
    ...
    this.position_topicAction = function () {
        var form = new Form(
            'message_form',
            {
                ...
                success: function (response) {
                    document.location = '/team/position_topic?id=' + response.id + '&last=1';
                }
            }
        );
    };
    ...
    this.deletePositionMessage = function (id) {
        if (confirm(this.l_confirm)) {
            ...
            $.post('/team/position_message_delete', {id: id});
        }
    };

    this.deletePositionTopic = function (id) {
        if (confirm(this.l_confirm)) {
            document.location = '/team/position_topic_delete?id=' + id;
        }
    };
...
//
    this.proposalAction = function () {
        var form = new Form(
            'topic_form',
            {
                ...
                success: function (response) {
                    document.location = '/team/proposal_topic?id=' + response.id;
                }
            }
        );
    };
    ...
    this.proposal_topicAction = function () {
        var form = new Form(
            'message_form',
            {
                validators: {text: [validatorRequired]},
                success: function (response) {
                    document.location = '/team/proposal_topic?id=' + response.id + '&last=1';
                }
            }
        );
    };
    ...
    this.deleteProposalMessage = function (id) {
        if (confirm(this.l_confirm)) {
            $('#talk_message' + id).fadeOut(150);
            $.post('/team/proposal_message_delete', {id: id});
        }
    };

    this.deleteProposalTopic = function (id) {
        if (confirm(this.l_confirm)) {
            document.location = '/team/proposal_topic_delete?id=' + id;
        }
    };
//
    ...
    this.talk_topicAction = function () {
        var form = new Form(
            'message_form',
            {
                validators: {text: [validatorRequired]},
                success: function (response) {
                    document.location = '/team/talk_topic?id=' + response.id + '&last=1';
                }
            }
        );
    };
    ...
    this.deleteTalkMessage = function (id) {
        if (confirm(this.l_confirm)) {
            $('#talk_message' + id).fadeOut(150);
            $.post('/team/talk_message_delete', {id: id});
        }
    };

    this.deleteTalkTopic = function (id) {
        if (confirm(this.l_confirm)) {
            document.location = '/team/talk_topic_delete?id=' + id;
        }
    };
    ...
    this.addModerator = function (id) {
        ...
        $.post(
            '/team/add_moderator',
            {id: $('#new_moderator').val(), group_id: teamController.groupId},
            function (response) {
                $('#no_moderators').hide();
    ...
    this.deleteModerator = function (object) {
        ...
        $.post(
            '/team/delete_moderator',
            {id: $(object).attr('rel'), group_id: teamController.groupId}
        );
    }
    ...
    this.changeOwner = function (id, object) {
        if (confirm($(object).attr('rel'))) {
            $.post(
                '/team/change_owner',
                {id: id, group_id: teamController.groupId},
                function (response) {
                    document.location = '/group' + teamController.groupId;
                }
            );
        }
    };
    ...
    this.vote = function (positive) {
        ...
        if (positive) {
            $.post(
                '/blogs/vote',
                {id: teamController.postId, positive: positive ? 1 : 0},
                function () {
