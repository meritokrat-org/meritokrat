var reformController = new function () {
		...
		var form = new Form(
			'add_form',
			{
				...
				success: function (response) {
					...
					if (response.success > 0)document.location = '/reform/create?success=' + response.id;
					else document.location = '/reform/edit?id=' + response.id + '&tab=photo';
				},
		...
		var commonForm = new Form(
			'common_form',
			{
				...
				success: function (response) {
					...
					document.location = '/reform/edit?id=' + response.id;
				},
		...
		var leadershipForm = new Form(
			'leadership_form',
			{
				...
				success: function (response) {
					...
					document.location = '/reform/edit?id=' + response.id + '&tab=leadership';
				},
	...
	//			success: function( response )
	//			{
	///				document.location = '/reform/talk_topic?id=' + response.id;
	//			}
	//		}
	//	);
	//};
	...
	this.positionAction = function () {
		var form = new Form(
			'topic_form',
			{
				...
				success: function (response) {
					document.location = '/reform/position_topic?id=' + response.id;
				}
			}
		);
	};
	...
	this.position_topicAction = function () {
		var form = new Form(
			'message_form',
			{
				...
				success: function (response) {
					document.location = '/reform/position_topic?id=' + response.id + '&last=1';
				}
			}
		);
	};
	...
	this.deletePositionMessage = function (id) {
		if (confirm(this.l_confirm)) {
			...
			$.post('/reform/position_message_delete', {id: id});
		}
	};

	this.deletePositionTopic = function (id) {
		if (confirm(this.l_confirm)) {
			document.location = '/reform/position_topic_delete?id=' + id;
		}
	};
...
//
	this.proposalAction = function () {
		var form = new Form(
			'topic_form',
			{
				...
				success: function (response) {
					document.location = '/reform/proposal_topic?id=' + response.id;
				}
			}
		);
	};
	...
	this.proposal_topicAction = function () {
		var form = new Form(
			'message_form',
			{
				validators: {text: [validatorRequired]},
				success: function (response) {
					document.location = '/reform/proposal_topic?id=' + response.id + '&last=1';
				}
			}
		);
	};
	...
	this.deleteProposalMessage = function (id) {
		if (confirm(this.l_confirm)) {
			$('#talk_message' + id).fadeOut(150);
			$.post('/reform/proposal_message_delete', {id: id});
		}
	};

	this.deleteProposalTopic = function (id) {
		if (confirm(this.l_confirm)) {
			document.location = '/reform/proposal_topic_delete?id=' + id;
		}
	};
//
	...
	this.talk_topicAction = function () {
		var form = new Form(
			'message_form',
			{
				validators: {text: [validatorRequired]},
				success: function (response) {
					document.location = '/reform/talk_topic?id=' + response.id + '&last=1';
				}
			}
		);
	};
	...
	this.deleteTalkMessage = function (id) {
		if (confirm(this.l_confirm)) {
			$('#talk_message' + id).fadeOut(150);
			$.post('/reform/talk_message_delete', {id: id});
		}
	};

	this.deleteTalkTopic = function (id) {
		if (confirm(this.l_confirm)) {
			document.location = '/reform/talk_topic_delete?id=' + id;
		}
	};
	...
	this.addModerator = function (id) {
		...
		$.post(
			'/reform/add_moderator',
			{id: $('#new_moderator').val(), group_id: reformController.groupId},
			function (response) {
				$('#no_moderators').hide();
				$('#moderators').append(response);
				$('#new_moderator').val('');
			}
		);
	}
	...
	this.deleteModerator = function (object) {
		...
		$.post(
			'/reform/delete_moderator',
			{id: $(object).attr('rel'), group_id: reformController.groupId}
		);
	}
	...
	this.changeOwner = function (id, object) {
		if (confirm($(object).attr('rel'))) {
			$.post(
				'/reform/change_owner',
				{id: id, group_id: reformController.groupId},
				function (response) {
					document.location = '/group' + reformController.groupId;
				}
			);
		}
	};
	...
	this.vote = function (positive) {
		...
		if (positive) {
			$.post(
				'/blogs/vote',
				{id: reformController.postId, positive: positive ? 1 : 0},
				function () {
